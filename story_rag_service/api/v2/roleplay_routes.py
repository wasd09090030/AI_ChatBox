...
async def list_personas(roleplay_services: RoleplayDependencies = Depends(get_roleplay_dependencies)):
    """返回人格卡列表。"""
    return roleplay_services.roleplay_manager.list_personas()
...
async def create_persona(
    ...
    """创建人格卡。"""
    return roleplay_services.roleplay_manager.create_persona(payload)


@router.get("/roleplay/personas/{persona_id}", response_model=PersonaProfile)
async def get_persona(persona_id: str, roleplay_services: RoleplayDependencies = Depends(get_roleplay_dependencies)):
    """按 ID 获取人格卡。"""
    persona = roleplay_services.roleplay_manager.get_persona(persona_id)
    if persona is None:
...
async def update_persona(
    ...
    """更新人格卡。"""
    persona = roleplay_services.roleplay_manager.update_persona(persona_id, payload)
    if persona is None:
        raise HTTPException(status_code=404, detail="Persona not found")
    return persona
...
async def delete_persona(persona_id: str, roleplay_services: RoleplayDependencies = Depends(get_roleplay_dependencies)):
    """删除人格卡。"""
    deleted = roleplay_services.roleplay_manager.delete_persona(persona_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Persona not found")
...
async def get_story_state(session_id: str, roleplay_services: RoleplayDependencies = Depends(get_roleplay_dependencies)):
    """按会话 ID 读取故事状态。"""
    state = roleplay_services.roleplay_manager.get_story_state(session_id)
    if state is None:
...
async def upsert_story_state(
    ...
    """按会话 ID 新增或更新故事状态。"""
    return roleplay_services.roleplay_manager.upsert_story_state(session_id, payload)
