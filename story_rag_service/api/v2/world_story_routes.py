...
async def list_worlds(
    ...
    """列出全部世界。"""
    return world_services.world_app.list_worlds(owner_user_id=current_user.id)
...
async def create_world(
    ...
    """创建世界。"""
    return world_services.world_app.create_world(world_data, owner_user_id=current_user.id)
...
async def get_world(
    ...
    """按 world_id 查询世界详情。"""
    world = world_services.world_app.get_world(world_id, owner_user_id=current_user.id)
    if world is None:
...
async def update_world(
    ...
    """更新世界。"""
    world = world_services.world_app.update_world(world_id, world_data, owner_user_id=current_user.id)
    if world is None:
        raise HTTPException(status_code=404, detail="World not found")
    return world
...
async def delete_world(
    ...
    """删除世界并触发关联数据级联清理。"""
    result = world_services.world_app.delete_world(world_id, owner_user_id=current_user.id)
    if result is None:
        raise HTTPException(status_code=404, detail="World not found")
    return result
...
async def list_stories(
    ...
    """按可选 world_id 列出故事。"""
    return world_services.story_manager.list_stories(world_id=world_id, owner_user_id=current_user.id)
...
async def create_story(
    ...
    """创建故事。"""
    world = world_services.world_manager.get_world(story_data.world_id, owner_user_id=current_user.id)
    if world is None:
        raise HTTPException(status_code=404, detail="World not found")
    return world_services.story_manager.create_story(story_data, world.name, owner_user_id=current_user.id)
...
async def get_story(
    ...
    """按 story_id 查询故事。"""
    story = world_services.story_manager.get_story(story_id, owner_user_id=current_user.id)
    if story is None:
...
async def update_story(
    ...
    """更新故事元数据。"""
    story = world_services.story_manager.update_story(story_id, update_data, owner_user_id=current_user.id)
    if story is None:
        raise HTTPException(status_code=404, detail="Story not found")
    return story
...
async def delete_story(
    ...
    """删除故事并清理对应实体状态快照。"""
    deleted = world_services.story_manager.delete_story(story_id, owner_user_id=current_user.id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Story not found")
...
async def add_story_segment(
    ...
    """追加一个故事片段。"""
    story = world_services.story_manager.add_segment(story_id, segment_data, owner_user_id=current_user.id)
    if story is None:
        raise HTTPException(status_code=404, detail="Story not found")
    return story
...
async def delete_last_story_segment(
    ...
    existing_story = world_services.story_manager.get_story(story_id, owner_user_id=current_user.id)
    ...
    try:
        story = world_services.story_manager.remove_last_segment(story_id, owner_user_id=current_user.id)
    ...
    if restored_runtime is not None:
        world_services.story_runtime_manager.sync_story_metadata(restored_runtime, owner_user_id=current_user.id)
        story = world_services.story_manager.get_story(story_id, owner_user_id=current_user.id) or story
...
async def commit_story_adjustments(
    ...
    try:
        updated_story = world_services.story_manager.update_story_segments_content(
            story_id,
            commit_data.updates,
            owner_user_id=current_user.id,
        )
        ...
        try:
            world_services.story_manager.save_story(existing_story, owner_user_id=current_user.id)
...
async def update_story_progress(
    ...
    """更新故事剧本推进元数据。"""
    story = world_services.story_manager.update_story_progress(
        story_id,
        progress_data,
        owner_user_id=current_user.id,
    )
...
async def get_story_runtime(
    ...
    runtime_state = world_services.story_runtime_manager.get_runtime_state(story_id, owner_user_id=current_user.id)
    if runtime_state is None:
        ...
        world_services.story_runtime_manager.sync_story_metadata(runtime_state, owner_user_id=current_user.id)
...
async def update_story_runtime(
    ...
    """更新故事运行时状态并回写故事进度元数据。"""
    try:
        runtime_state = world_services.story_runtime_manager.update_runtime_state(
            story_id,
            runtime_update,
            owner_user_id=current_user.id,
        )
