...
class LorebookSqliteStore:
    ...
    def upsert_entry(self, entry: LorebookEntry, chroma_ref: Optional[str] = None) -> None:
        """写入或覆盖 lorebook 条目。
        ...
        with self.connect() as conn:
            conn.execute(
                ...
                INSERT INTO lorebook_entries
                    (id, world_id, type, name, content, keywords, trigger_keywords,
    ...
    def list_entries(self, world_id: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        with self.connect() as conn:
            if world_id:
                ...
                rows = conn.execute("SELECT * FROM lorebook_entries").fetchall()
