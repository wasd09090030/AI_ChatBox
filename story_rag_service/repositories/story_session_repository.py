...
class StorySessionRepository:
    ...
    def mark_first_message_sent(self, session_id: str, owner_user_id: Optional[str] = None) -> bool:
        ...
        with self._connect() as conn:
            if owner_user_id is None:
                ...
                cursor = conn.execute(
                    "UPDATE story_sessions SET first_message_sent = 1 WHERE session_id = ? AND owner_user_id = ?",
                    (session_id, owner_user_id),
                )
            conn.commit()
    ...
    def set_first_message_sent(self, session_id: str, sent: bool, owner_user_id: Optional[str] = None) -> bool:
        ...
        with self._connect() as conn:
            if owner_user_id is None:
                ...
                cursor = conn.execute(
                    "UPDATE story_sessions SET first_message_sent = ? WHERE session_id = ? AND owner_user_id = ?",
                    (1 if sent else 0, session_id, owner_user_id),
                )
            conn.commit()
            return cursor.rowcount > 0
    ...
    def replace_session_messages(
        ...
        with self._connect() as conn:
            ...
            delete_sql = "DELETE FROM story_session_messages WHERE session_id = ?"
            if owner_user_id is not None:
                delete_sql += (
                    " AND EXISTS (SELECT 1 FROM story_sessions s "
                    "WHERE s.session_id = story_session_messages.session_id AND s.owner_user_id = ?)"
                )
                delete_params = (session_id, owner_user_id)
            ...
            for item in messages:
                ...
                conn.execute(
                    ...
                    INSERT INTO story_session_messages
                        (id, session_id, owner_user_id, role, content, timestamp, token_estimate, archived)
                    VALUES (?, ?, ?, ?, ?, ?, ?, 0)
                    """,
                    (
                        str(uuid.uuid4()),
