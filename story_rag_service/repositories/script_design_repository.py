...
class ScriptDesignRepository:
    """剧本设计仓储抽象接口。"""
    def save(self, script_design: ScriptDesign, owner_user_id: Optional[str] = None) -> ScriptDesign:
        """新增或更新剧本设计记录，并返回最新对象。"""
        raise NotImplementedError

    def get(self, script_design_id: str, owner_user_id: Optional[str] = None) -> Optional[ScriptDesign]:
        """按剧本设计 ID 查询单条记录。"""
        raise NotImplementedError
    ...
    def delete(self, script_design_id: str, owner_user_id: Optional[str] = None) -> bool:
        """按 ID 删除剧本设计并返回是否成功。"""
        raise NotImplementedError
    ...
    def count(self, world_id: Optional[str] = None, owner_user_id: Optional[str] = None) -> int:
...
class SqliteScriptDesignRepository(ScriptDesignRepository):
    ...
    def _init_table(self):
        ...
        with self._connect() as conn:
            ...
            existing_columns = {
                row[1]
                for row in cursor.execute("PRAGMA table_info(script_designs)").fetchall()
            }
            if "owner_user_id" not in existing_columns:
                cursor.execute("ALTER TABLE script_designs ADD COLUMN owner_user_id TEXT DEFAULT NULL")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_script_designs_owner_user_id ON script_designs(owner_user_id)")
    ...
    def save(self, script_design: ScriptDesign, owner_user_id: Optional[str] = None) -> ScriptDesign:
        ...
        with self._connect() as conn:
            ...
            cursor.execute(
                """
                INSERT INTO script_designs (id, owner_user_id, world_id, status, updated_at, payload)
                VALUES (?, ?, ?, ?, ?, ?)
                ON CONFLICT(id) DO UPDATE SET
                    owner_user_id = COALESCE(excluded.owner_user_id, script_designs.owner_user_id),
    ...
    def get(self, script_design_id: str, owner_user_id: Optional[str] = None) -> Optional[ScriptDesign]:
        ...
        with self._connect() as conn:
            ...
            if owner_user_id is None:
                ...
                cursor.execute(
                    "SELECT payload FROM script_designs WHERE id = ? AND owner_user_id = ?",
                    (script_design_id, owner_user_id),
                )
            row = cursor.fetchone()
        if not row:
            return None
    ...
    def delete(self, script_design_id: str, owner_user_id: Optional[str] = None) -> bool:
        ...
        with self._connect() as conn:
            ...
            if owner_user_id is None:
                ...
                cursor.execute(
                    "DELETE FROM script_designs WHERE id = ? AND owner_user_id = ?",
                    (script_design_id, owner_user_id),
                )
    ...
    def delete_by_world(self, world_id: str, owner_user_id: Optional[str] = None) -> int:
        ...
        with self._connect() as conn:
            ...
            if owner_user_id is None:
                ...
                cursor.execute(
                    "DELETE FROM script_designs WHERE world_id = ? AND owner_user_id = ?",
