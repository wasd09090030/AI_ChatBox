...
class EntityStateCompanionRef(BaseModel):
    """实体同行关系的 API 输出引用。"""
...
class EntityStateSnapshotResponse(BaseModel):
    """单个故事实体的 API 输出快照。"""
    ...
    inventory: List[str] = Field(default_factory=list)
    status_tags: List[str] = Field(default_factory=list)
    companions: List[EntityStateCompanionRef] = Field(default_factory=list)
    short_goal: Optional[str] = None
    state_summary: Optional[str] = None
    evidence: List[str] = Field(default_factory=list)
    last_source_turn: Optional[int] = None
...
class EntityStateCollectionResponse(BaseModel):
    """故事/会话维度实体状态 API 响应。"""

    story_id: Optional[str] = None
    session_id: str
    entity_type: Optional[EntityType] = None
    items: List[EntityStateSnapshotResponse] = Field(default_factory=list)
    total: int = 0
...
class EntityStateRebuildResponsePayload(BaseModel):
    """实体状态重建 API 输出载荷。"""
